 ...
 before_filter :admin_required
  ...
  def update
    ...
    if  @dealing_center.update_attributes(params[:dealing_center])
      if params[:rate]=="true" && current_user.moderator? && !@dealing_center.temp? then  User.find(:first, :conditions => {:id => @dealing_center.added_by_user}).add_rating(Settings.rating.bonus.for_dealing_center,User.find(:first,:conditions=>{:admin=>true}),"за диллинговый центр") end
