class PagesController < ApplicationController
  ...
  def home
    ...
    @dealing_centers = DealingCenter.find_by_sql(" SELECT dc.id AS id, dc.name AS name, dc.url AS url FROM dealing_centers dc LEFT JOIN users u ON dc.id = u.dealing_center_id WHERE temp=false GROUP BY dc.name ORDER BY COUNT( dc.id ) DESC , dc.name LIMIT 0 , 30 ")
